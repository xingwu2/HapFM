...
import HaploSpace_g as hs
import HaploSpace_g_auto as hs_auto
...
def custom_fine_partition(block):
	...
	with open(block, "r") as f:
		for line in f:
			...
			if ch in fine_breakpoints:
				fine_breakpoints[ch][i] = []
			...
			for j in range(1,len(items)):
				m = re.search("\[(.*)\]",items[j])
				if "," in m.group(1):
					n = re.search("(.*), (.*)",m.group(1))
					fine_breakpoints[ch][i].append([int(n.group(1)),int(n.group(2))])
				else:
					fine_breakpoints[ch][i].append([int(m.group(1))])
			i += 1
